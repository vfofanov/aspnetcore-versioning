...
namespace Microsoft.AspNet.OData
{
    ...
    public sealed class DefaultModelTypeBuilder : IModelTypeBuilder
    {
        ...
        private readonly ConcurrentDictionary<ApiVersion, ConcurrentDictionary<EdmTypeKey, Type>> _generatedActionParamsPerVersion = new();
        private readonly ConcurrentDictionary<ApiVersion, IDictionary<EdmTypeKey, Type>> _generatedEdmTypesPerVersion = new();
        private readonly ConcurrentDictionary<ApiVersion, ModuleBuilder> _modules = new();
        ...
        public Type NewStructuredType(IEdmStructuredType structuredType, Type clrType, ApiVersion apiVersion, IEdmModel edmModel)
        {
            ...
            var edmTypes = _generatedEdmTypesPerVersion.GetOrAdd(apiVersion, key => GenerateTypesForEdmModel(edmModel, key));
        ...
        public Type NewActionParameters(IServiceProvider services, IEdmAction action, ApiVersion apiVersion, string controllerName)
        {
            ...
            var paramTypes = _generatedActionParamsPerVersion.GetOrAdd(apiVersion, _ => new ConcurrentDictionary<EdmTypeKey, Type>());
            ...
            var type = paramTypes.GetOrAdd(key, _ =>
            {
                ...
                var moduleBuilder = _modules.GetOrAdd(apiVersion, CreateModuleForApiVersion);
        ...
        private IDictionary<EdmTypeKey, Type> GenerateTypesForEdmModel(IEdmModel model, ApiVersion apiVersion)
        {
            ...
            {
                return _modules.GetOrAdd(apiVersion, CreateModuleForApiVersion);
            }
            ...
            foreach (var property in clrType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (structuredTypeRef.IsCollection())
                {
                    ...
                    if (elementType.IsStructured())
                    {
                        ...
                        {
                            propertyType = EnumerableOfT.MakeGenericType(newItemType);
                            clrTypeMatchesEdmType = false;
                        }
                    }
                }
        ...
        private static Type CreateTypeInfoFromSignature(ModuleBuilder moduleBuilder, ClassSignature @class)
        {
            return CreateTypeBuilderFromSignature(moduleBuilder, @class).CreateType()!;
        }
        ...
        private static IDictionary<EdmTypeKey, Type> ResolveDependencies(BuilderContext context)
        {
            ...
            for (var i = 0; i < dependencies.Count; i++)
            {
                ...
                {
                    dependentOnType = EnumerableOfT.MakeGenericType(dependentOnType).GetTypeInfo();
                }
        ...
        {
            var name = new AssemblyName($"T{Guid.NewGuid().ToString("n", CultureInfo.InvariantCulture)}.DynamicModels");
        ...
        private sealed class BuilderContext
        {
            ...
            internal BuilderContext(IEdmModel edmModel, ApiVersion apiVersion, Func<ModuleBuilder> moduleBuilderFactory)
            {
                ...
                _moduleBuilder = new Lazy<ModuleBuilder>(moduleBuilderFactory);
            }

            internal ModuleBuilder ModuleBuilder => _moduleBuilder.Value;
